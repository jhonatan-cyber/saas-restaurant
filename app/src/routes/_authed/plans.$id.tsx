...
function PlanEditPage(): ReactNode {
  ...
  if (planQuery.data && !form) {
    setForm({
      ...
      selectedFeatures: planQuery.data.features ?? [],
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      {form && (
        <form onSubmit={handleSubmit} className="card p-6 space-y-6">
          ...
          {/* Features */}
          <div>
            <label className="label">Features del plan</label>
            <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">
              {PLAN_FEATURE_OPTIONS.map((feature) => {
                const checked = form.selectedFeatures.includes(feature.code);
                return (
                  <label
                    key={feature.code}
                    ...
                    <input
                      ...
                      onChange={() => {
                        update(
                          'selectedFeatures',
                          checked
                            ? form.selectedFeatures.filter((f) => f !== feature.code)
                            : [...form.selectedFeatures, feature.code],
                        );
                      }}
                    />
                    {feature.label}
            ...
            <p className="mt-1 text-xs text-slate-400">
              {form.selectedFeatures.length} de {PLAN_FEATURE_OPTIONS.length} seleccionados
            </p>
          </div>
