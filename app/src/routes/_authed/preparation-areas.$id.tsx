...
function EditPrepAreaPage(): ReactNode {
  ...
  const areaQuery = useQuery({
    queryKey: ['preparation-areas', id],
    queryFn: () => preparationAreasApi.get(id),
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (data: PreparationAreaFormValues) => {
      ...
      return preparationAreasApi.update(id, payload);
    },
    onSuccess: () => {
  ...
  if (areaQuery.error) {
    return <p className="text-red-600">Error: {String(areaQuery.error)}</p>;
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-bold text-slate-900">Editar área</h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="card p-6 space-y-4" noValidate>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <FormField label="Sucursal" htmlFor="branchId">
            <select id="branchId" className="input" {...register('branchId')} disabled>
