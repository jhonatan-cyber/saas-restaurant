...
/**
 * Valida y normaliza los search params del login.
 * Exportada para testing.
 */
export function validateSearch(search: Record<string, unknown>): LoginSearchParams {
  return {
    slug: typeof search.slug === 'string' ? search.slug.trim() : undefined,
...
function SpinnerIcon() {
  return <OrbSpinner size={20} state="working" speed={3.00} />;
}
...
export function getLoginErrorMessage(err: unknown): { title: string; description: string } {
...
/**
 * @param searchParams - Opcional para testing; en runtime usa Route.useSearch().
 */
export function LoginPage({ searchParams }: { searchParams?: LoginSearchParams } = {}): ReactNode {
  ...
  const mounted = true;
  const search: LoginSearchParams = searchParams ?? Route.useSearch();
  const slug = search.slug;
  const redirectTo = search.redirect;

  const formDefaults = {
    ...loginFormDefaults,
    businessSlug: slug || loginFormDefaults.businessSlug || '',
  };
  ...
  const {
    ...
    defaultValues: formDefaults as LoginFormValues,
  });
  ...
  const onSubmit = async (values: LoginFormValues): Promise<void> => {
    ...
    try {
      ...
      // Si hay ?redirect=, ir a esa URL (intra-app); si no, al dashboard
