...
function KdsPage(): ReactNode {
  ...
  const transitionMutation = useMutation({
    mutationFn: async ({ orderId, to, expectedVersion }: { orderId: string; to: OrderStatus; expectedVersion: number }) => {
      return ordersApi.transition(orderId, { to, expectedVersion });
    },
    onSuccess: () => {
              ...
              {area.orders.length === 0 ? (
                ...
                area.orders.map((order: KdsOrder) => (
                  <KdsOrderCard
                    ...
                    onTransition={(to) => {
                      transitionMutation.mutate({ orderId: order.id, to, expectedVersion: order.version });
                    }}
                    onCancel={(reason) => {
