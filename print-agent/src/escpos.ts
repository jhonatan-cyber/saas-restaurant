...
/**
 * Fila de sub-item para ticket (producto incluido en un combo, SIN cargo extra).
 * Se muestra indentado con precio 0.00.
 * Ej: "    · 1× Hamburguesa                       0.00"
 */
export function ticketSubRow(
  name: string,
  qty: number,
): Uint8Array {
  const left = `    · ${qty}× ${name}`;
  const maxLeft = Math.min(LINE_WIDTH - 16, left.length);
  const display = left.slice(0, maxLeft).padEnd(maxLeft);
  const combined = `${display}${' '.repeat(LINE_WIDTH - maxLeft - 14)}${'0.00'.padStart(14)}`;
  return concat(encodeText(combined), lf());
}
...
/**
 * Fila de sub-item (producto incluido en un combo) para comanda.
 * Se muestra indentado y sin precio, ej: "    · 1× Hamburguesa"
 */
